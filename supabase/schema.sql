...
-- ==========================================
-- AI PROVIDERS TABLE (Task 1)
-- ==========================================
-- api_key stored server-side only (never returned to client via RLS)
...
-- Only admin role via authenticated client (api_key excluded in server SELECT)
...
CREATE POLICY "HR manages own upload links" ON public.upload_links
  FOR ALL TO authenticated
  USING (hr_user_id = auth.uid())
  WITH CHECK (hr_user_id = auth.uid());

CREATE POLICY "Admins view all upload links" ON public.upload_links
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'));

-- RPC function: atomically increment use_count (called by service role, no client access)
CREATE OR REPLACE FUNCTION increment_upload_link_use_count(link_id BIGINT)
RETURNS void LANGUAGE sql SECURITY DEFINER AS $$
  UPDATE public.upload_links SET use_count = use_count + 1 WHERE id = link_id;
$$;
