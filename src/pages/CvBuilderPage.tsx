...
const translations = {
  en: {
    ...
    importing: "Importing...",
    importConfirm:
      "This will replace all current data in the form. Are you sure you want to continue?",
    importSuccess: "CV data imported successfully!",
    importError:
      "Failed to import CV. Please try another file or enter the data manually.",
    ...
    guideIntro:
      "Here are key tips from best practices to create a standout, ATS-friendly CV:",
  ...
  ar: {
    ...
    importConfirm:
      "سيؤدي هذا إلى استبدال جميع البيانات الحالية في النموذج. هل أنت متأكد من أنك تريد المتابعة؟",
    importSuccess: "تم استيراد بيانات السيرة الذاتية بنجاح!",
    importError:
      "فشل استيراد السيرة الذاتية. يرجى تجربة ملف آخر أو إدخال البيانات يدويًا.",
    ...
    guideIntro:
      "إليك أهم النصائح من أفضل الممارسات لإنشاء سيرة ذاتية مميزة ومتوافقة مع أنظمة تتبع المتقدمين (ATS):",
...
const initialCvData = {
  // Intentionally blank: visitors start from their own information rather than
  // seeing a demo person's personal data in the builder or exported PDF.
  name: "",
  jobTitle: "",
  email: "",
  phone: "",
...
// New key intentionally ignores old browser-stored demo content from versions
// that shipped a sample CV. Existing visitors now open a clean builder.
const CV_DATA_STORAGE_KEY = "cv-builder-data-v2";
...
const DEFAULT_SECTIONS = [
  {
    ...
    labelAr: "الملخص الاحترافي",
    visible: true,
  },
  {
    key: "experience",
    labelEn: "Work Experience",
    labelAr: "الخبرة العملية",
    visible: true,
  },
    ...
    setCvData((prev) => {
      const newId =
        prev[section].length > 0
          ? Math.max(...prev[section].map((i) => i.id)) + 1
          : 1;
      let newItem;
      switch (section) {
        case "experience":
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const processedData = {
            ...structuredClone(initialCvData),
            ...parsedData,
            experience: (parsedData.experience || []).map((item, index) => ({
              ...item,
              id: index + 1,
            })),
            education: (parsedData.education || []).map((item, index) => ({
              ...item,
              id: index + 1,
            })),
            skills: (parsedData.skills || []).map((item, index) => ({
              ...item,
              id: index + 1,
            })),
            ...
            links: (parsedData.links || []).map((item, index) => ({
              ...item,
              id: index + 1,
            })),
          };
        ...
        <div className="cv-form">
          <FormSection title={t.layoutOptions}>
            ...
            <div className="section-controls-wrapper">
              <label className="section-controls-label">
                {t.sectionVisibility}
              </label>
              <div className="section-controls-list">
                {(sections || DEFAULT_SECTIONS).map((sec, index) => (
                  ...
                  >
                    <span className="drag-handle" title={t.sectionOrder}>
                      ⠿
                    </span>
                    ...
                    <span
                      className={`section-name${!sec.visible ? " section-name-hidden" : ""}`}
                    >
                    ...
                    <div className="order-btns">
                      ...
                      <button
                        ...
                        disabled={
                          index === (sections || DEFAULT_SECTIONS).length - 1
                        }
                        title="Move down"
                      >
                        {t.moveDown}
            ...
            {(cvData.links || []).map((link) => (
              <div key={link.id} className="dynamic-item">
                <InputField
                  ...
                  name="text"
                  value={link.text}
                  onChange={(e) => handleDynamicChange("links", link.id, e)}
                />
                <InputField
                  label={t.url}
                  name="url"
                ...
                <div className="dynamic-controls">
                  <button
                    ...
                    onClick={() => removeDynamicItem("links", link.id)}
                  >
                    {t.remove}
                  </button>
                </div>
          ...
          <FormSection title={t.workExperience}>
            {cvData.experience.map((exp) => (
              <div key={exp.id} className="dynamic-item">
                <InputField
                  ...
                  name="title"
                  value={exp.title}
                  onChange={(e) => handleDynamicChange("experience", exp.id, e)}
                />
                <InputField
                  label={t.company}
                  name="company"
            ...
            <div className="dynamic-controls">
              <button
                className="add-btn"
                onClick={() => addDynamicItem("experience")}
              >
                {t.addExperience}
              </button>
            </div>
          ...
          <FormSection title={t.education}>
            {cvData.education.map((edu) => (
              <div key={edu.id} className="dynamic-item">
                <InputField
                  ...
                  name="degree"
                  value={edu.degree}
                  onChange={(e) => handleDynamicChange("education", edu.id, e)}
                />
                <InputField
                  label={t.university}
                  name="university"
            ...
            <div className="dynamic-controls">
              <button
                className="add-btn"
                onClick={() => addDynamicItem("education")}
              >
                {t.addEducation}
              </button>
            </div>
            ...
            {cvData.skills.map((skill) => (
              <div key={skill.id} className="dynamic-item">
                <InputField
                  ...
                  name="text"
                  value={skill.text}
                  onChange={(e) => handleDynamicChange("skills", skill.id, e)}
                />
                <div className="dynamic-controls">
                  <button
                    className="remove-btn"
                    onClick={() => removeDynamicItem("skills", skill.id)}
                  >
                    {t.remove}
                  </button>
                </div>
          ...
          <FormSection title={t.certifications}>
            {cvData.certifications.map((cert) => (
              <div key={cert.id} className="dynamic-item">
                <InputField
                  ...
                  value={cert.text}
                  onChange={(e) =>
                    handleDynamicChange("certifications", cert.id, e)
                  }
                />
                <div className="dynamic-controls">
                  <button
                    className="remove-btn"
                    onClick={() => removeDynamicItem("certifications", cert.id)}
                  >
                    {t.remove}
                  </button>
                </div>
            ...
            <div className="dynamic-controls">
              <button
                className="add-btn"
                onClick={() => addDynamicItem("certifications")}
              >
                {t.addCertification}
              </button>
            </div>
        ...
        <div className="cv-preview-container">
          <div
            ...
            className={[
              ...
              .join(" ")}
            style={
              { "--base-font-size": `${fontSize}pt` } as React.CSSProperties
            }
          >
            <div className="preview-header">
              ...
              <p className="preview-contact">
                {cvData.address || t.address} | {cvData.email || t.email} |{" "}
                {cvData.phone || t.phone}
                {(cvData.links || [])
                  .filter((l) => l.url)
                  .map((link) => (
                          ...
                          <div key={exp.id} className="experience-item">
                            <div className="item-header">
                              <span className="title">
                                {exp.title || t.preview_job_title}
                              </span>
                              <span className="date">
                                {exp.from || t.preview_from} –{" "}
                                {exp.to || t.preview_to}
                              </span>
                            </div>
                            <div className="item-subheader">
                              {exp.company || t.preview_company},{" "}
                              {exp.location || t.preview_location}
                            </div>
                          ...
                          <div key={edu.id} className="education-item">
                            <div className="item-header">
                              <span className="degree">
                                {edu.degree || t.preview_degree}
                              </span>
                              <span className="date">
                                {t.preview_graduated}:{" "}
                                {edu.year || t.preview_year}
                              </span>
                            </div>
                            <div className="item-subheader">
                              {edu.university || t.preview_university},{" "}
                              {edu.location || t.preview_location}
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div key="certifications" className="preview-section">
                      ...
                      <div className="content certifications-list">
                        <ul>
                          {cvData.certifications.map((cert) => (
                            <li key={cert.id}>
                              {cert.text || t.preview_certification}
                            </li>
                          ))}
                        </ul>
                      </div>
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <ul>
          {t.guideStructure.map((item, index) => (
...
const CVStrengthTesterModal = ({ isVisible, onClose, t, cvData }) => {
  ...
  return (
    <div className="modal-overlay" onClick={handleClose}>
      <div
        className="modal-content tester-modal-content"
      ...
      >
        ...
        {analysisResult && (
          <div className="analysis-results">
            ...
            <div className="results-grid">
              <div className="score-container">
                ...
                <div
                  ...
                  style={
                    { "--score": analysisResult.score } as React.CSSProperties
                  }
                >
            ...
            <button className="analyze-btn" onClick={handleClose}>
              Close
            </button>
          </div>
        )}
