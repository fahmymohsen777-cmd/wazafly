...
const translations = {
  en: {
    ...
    generateReportButton: (count) =>
      count > 0 ? `Generate Report for ${count} Selected` : "Generate Report",
    matchError:
      "Please enter a job description and select at least one resume to generate a report.",
    scoreError: "Please enter a job description to calculate scores.",
    reportGenerationError:
      "Report generation failed. The AI model returned an empty or invalid response. This may be due to content safety filters or a temporary service issue.",
    ...
    analysisError: (fileName) =>
      `An error occurred while analyzing: ${fileName}`,
    viewCV: "View CV",
    downloadCV: "Download CV",
    moveCV: "Move CV",
    apiKeyError:
      "AI Service could not be initialized. Please ensure the API key is configured correctly in the environment.",
    supabaseError:
      "Could not connect to the CV Bank. Please check the configuration.",
    supabaseBucketNotFound: (bucketName) =>
      `Storage bucket "${bucketName}" not found. Please create a public bucket with this exact name in your Supabase project's Storage section.`,
    supabaseRLSError:
      "Permission Denied: Your database security policy is blocking new uploads. Please run the required SQL commands in your Supabase SQL Editor to grant access.",
    ...
    deleteConfirm: (name) =>
      `Are you sure you want to permanently delete the resume for ${name}? This action cannot be undone.`,
    ...
    deleteFolderConfirm: (name) =>
      `Are you sure you want to delete the folder "${name}"? Resumes inside it will become unassigned.`,
  ...
  ar: {
    ...
    generateReportButton: (count) =>
      count > 0 ? `إنشاء تقرير لـ ${count} محددين` : "إنشاء تقرير",
    matchError:
      "يرجى إدخال وصف وظيفي واختيار مرشح واحد على الأقل لإنشاء تقرير.",
    scoreError: "يرجى إدخال وصف وظيفي لحساب درجات التقييم.",
    reportGenerationError:
      "فشل إنشاء التقرير. أعاد نموذج الذكاء الاصطناعي استجابة فارغة أو غير صالحة. قد يكون هذا بسبب مرشحات أمان المحتوى أو مشكلة مؤقتة في الخدمة.",
    ...
    analysisPrompt:
      "حلل هذه السيرة الذاتية واستخرج المعلومات التالية. أجب باللغة العربية.",
    analysisError: (fileName) => `حدث خطأ أثناء تحليل الملف: ${fileName}`,
    viewCV: "عرض السيرة الذاتية",
    downloadCV: "تحميل السيرة الذاتية",
    moveCV: "نقل السيرة الذاتية",
    apiKeyError:
      "تعذر تهيئة خدمة الذكاء الاصطناعي. يرجى التأكد من تكوين مفتاح الواجهة البرمجية بشكل صحيح في البيئة.",
    supabaseError:
      "لا يمكن الاتصال ببنك السير الذاتية. يرجى التحقق من الإعدادات.",
    supabaseBucketNotFound: (bucketName) =>
      `حاوية التخزين "${bucketName}" غير موجودة. يرجى إنشاء حاوية تخزين عامة (public bucket) بهذا الاسم تمامًا في قسم التخزين بمشروعك على Supabase.`,
    supabaseRLSError:
      "تم رفض الإذن: سياسة الأمان في قاعدة البيانات تمنع رفع الملفات. يرجى تشغيل أوامر SQL اللازمة في محرر SQL بمشروعك على Supabase لمنح الصلاحيات.",
    loadingCVBank: "جاري تحميل بنك السير الذاتية...",
    uploadingToBank: (fileName) =>
      `جاري حفظ ${fileName} في بنك السيرة الذاتية...`,
    ...
    deleteConfirm: (name) =>
      `هل أنت متأكد من رغبتك في حذف السيرة الذاتية لـ ${name} نهائياً؟ لا يمكن التراجع عن هذا الإجراء.`,
    ...
    deleteFolderConfirm: (name) =>
      `هل أنت متأكد من حذف المجلد "${name}"؟ ستصبح السير الذاتية بداخله غير مصنفة.`,
  ...
  while (attempt < retries) {
    try {
      ...
      if (isRetriable && attempt < retries) {
        ...
        console.warn(
          `Retriable error detected. Retrying in ${(delay + jitter) / 1000}s... (Attempt ${attempt})`,
          e,
        );
...
const CVBankApp = ({
  ...
  const [currentResumeForComments, setCurrentResumeForComments] =
    useState(null);
  ...
  const reportLoadingMessages = {
    en: [
      ...
      "Comparing candidate summaries...",
      "Identifying strengths & weaknesses...",
      "Formatting final report...",
    ],
    ar: [
      "جاري تحليل الوصف الوظيفي...",
      "مقارنة ملخصات المرشحين...",
  ...
  useEffect(() => {
    if (isLoading && loadingTask) {
      const messages =
        loadingTask === "reporting"
          ? reportLoadingMessages[lang]
          : scoreLoadingMessages[lang];
      let messageIndex = 0;
  ...
  const loadAppData = useCallback(async () => {
    ...
    try {
      ...
      if (resumesResponse.error)
        throw new Error(`Resumes: ${resumesResponse.error.message}`);
      if (foldersResponse.error)
        throw new Error(`Folders: ${foldersResponse.error.message}`);
      if (commentsResponse.error)
        throw new Error(`Comments: ${commentsResponse.error.message}`);

      const pDict = {};
      if (profilesResponse.data) {
        profilesResponse.data.forEach((p) => {
          pDict[p.user_id] = p.name;
        });
      }
      ...
      const sortedFolders = safeFoldersData.sort((a, b) =>
  ...
  useEffect(() => {
    if (supabaseClient && session) {
      // Only load data if there is a session
      loadAppData();
    }
  ...
  const resumeSchema = {
    ...
    properties: {
      name: {
        type: Type.STRING,
        description: "Candidate's full name (Use Arabic if possible)",
      },
      age: { type: Type.NUMBER, description: "Candidate's age" },
      governorate: {
        type: Type.STRING,
        description:
          "The governorate or city where the candidate resides (in Arabic)",
      },
      email: { type: Type.STRING, description: "Candidate's email address" },
      phone: { type: Type.STRING, description: "Candidate's phone number" },
      appliedFor: {
        type: Type.STRING,
        description: "The job position applied for (in Arabic)",
      },
      skills: {
        type: Type.ARRAY,
        items: { type: Type.STRING },
        description: "A list of key skills (MUST BE IN ARABIC)",
      },
      aiSummary: {
        type: Type.STRING,
        description:
          "A comprehensive 3-4 sentence summary report of the CV, highlighting key skills, total years of experience, and their strongest qualifications. MUST BE WRITTEN IN ARABIC.",
      },
    },
  ...
  const handleFileDrop = useCallback(
    async (files) => {
      ...
      // File pickers on some mobile/Arabic Windows setups may provide a valid
      // MIME type while the filename extension is not exposed reliably. Accept
      // either a known extension OR a known MIME type for regular CV files.
      ...
      if (allFiles.length === 0) {
        setError(
          "لم يتم العثور على ملفات مدعومة (PDF, DOC, DOCX, XLS, XLSX, ZIP).",
        );
        setStatusMessage("");
        return;
      }
  ...
  useEffect(() => {
    ...
    if (!nextItem) {
      ...
      if (
        ...
        successCount + errorCount === uploadQueue.length
      ) {
        let finalMsg = "";
    ...
    const processItem = async (item) => {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "AI analysis failed, proceeding with default values.",
            analyzeError,
          );
          analysisData = {
        ...
        if (uploadError) {
          if (uploadError.message.toLowerCase().includes("bucket not found"))
            throw new Error(T.supabaseBucketNotFound("resumes"));
        ...
        const resumeForDb = {
          ...
          governorate: analysisData.governorate,
          email: analysisData.email,
          phone: analysisData.phone,
          applied_for: analysisData.appliedFor,
        ...
        if (insertError) {
          ...
          if (insertError.message.toLowerCase().includes("row-level security"))
            throw new Error(T.supabaseRLSError);
        ...
        const newResume = {
          ...
          age: insertData.age,
          governorate: insertData.governorate,
          email: insertData.email,
          phone: insertData.phone,
          appliedFor: insertData.applied_for,
          skills: insertData.skills,
          aiSummary: insertData.ai_summary,
          fileURL: insertData.file_url,
          fileName: insertData.file_name,
          folderId: insertData.folder_id,
  ...
  const handleDownloadResume = async (resumeToDownload) => {
    ...
    setCardActionStates((prev) => ({
      ...prev,
      [resumeToDownload.id]: "downloading",
    }));
    setError("");
  ...
  const handleDeleteResume = async (resumeToDelete) => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `Failed to delete file from storage: ${storageError.message}. Check your Storage RLS policies.`,
        );
      }
      ...
      if (dbError) {
        throw new Error(
          `Database record deletion failed: ${dbError.message}. Check your Table RLS policies.`,
        );
      }
      ...
      setTempScores((prev) => {
        const next = { ...prev };
        delete next[resumeToDelete.id];
        return next;
      });
      const successMsg = T.deleteSuccess;
      setStatusMessage(successMsg);
      setTimeout(
        () => setStatusMessage((prev) => (prev === successMsg ? "" : prev)),
        4000,
      );
    } catch (e) {
        ...
        items: {
          ...
          properties: {
            ...
            score: {
              type: Type.NUMBER,
              description:
                "Match score from 0 to 100 based on the job description.",
            },
            reason: {
              type: Type.STRING,
...
`;
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (error) {
            throw new Error(
              `Failed to delete folder: ${error.message}. Please check your RLS policies on the 'folders' table.`,
            );
          }
          ...
          setResumes((prev) =>
            prev.map((r) =>
              r.folderId === folder.id ? { ...r, folderId: null } : r,
            ),
          );
  ...
  const handleMoveResume = async (resumeId, folderId) => {
    ...
    try {
      ...
      if (error) throw error;
      setResumes((prev) =>
        prev.map((r) => (r.id === resumeId ? { ...r, folderId } : r)),
      );
    } catch (e) {
  ...
  const handleToggleFavorite = async (resumeToToggle) => {
    ...
    setResumes((prev) =>
      prev.map((r) =>
        r.id === id ? { ...r, is_favorited: !is_favorited } : r,
      ),
    );
    ...
    try {
      ...
      setResumes((prev) =>
        prev.map((r) => (r.id === id ? { ...r, is_favorited } : r)),
      );
    ...
    return resumesInScope
      .filter((r) => {
        ...
        const requiredSkills = filters.skills
          ...
          .filter(Boolean);
        const candidateSkills = Array.isArray(r.skills)
          ? r.skills
  ...
  const handleSelectAll = () => {
    ...
    const allCurrentlyVisibleAreSelected =
      allVisibleIds.length > 0 &&
      allVisibleIds.every((id) => selectedResumeIds.has(id));
  ...
  const UploadQueue = () => {
    ...
    const StatusIcon = ({ status, error }) => {
      const iconMap = {
        ...
        error: html` <div class="queue-item-error-tooltip">
          <svg
            class="status-error"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
    ...
    const isQueueFinished = uploadQueue.every(
      (item) => item.status === "success" || item.status === "error",
    );
    return html`<div class="upload-queue-container">
        <ul class="upload-queue-list">
          ${uploadQueue.map((item) => html`<li class="upload-queue-item" key=${item.id}><${StatusIcon} status=${item.status} error=${item.error} /><span class="queue-item-name">${item.file.name}</span></li>`)}
        </ul>
      </div>
      ${isQueueFinished && html`<div class="upload-queue-footer"><button class="clear-queue-btn" onClick=${() => setUploadQueue([])}>${T.clearQueue}</button></div>`}`;
  };
          ...
          if (line.startsWith("### ")) {
            ...
            const parts = line.split(/(\*\*.*?\*\*|\*\*.*)/).map((part) => {
              if (part.startsWith("**") && part.endsWith("**")) {
                return html`<strong
                  >${part.substring(2, part.length - 2)}</strong
                >`;
              }
              return part;
            });
    ...
    return html`
      <div class="report-modal-overlay" onClick=${onClose}>
        <div class="report-modal" onClick=${(e) => e.stopPropagation()}>
          ...
          <div class="report-modal-content">${renderMarkdown(content)}</div>
        </div>
      </div>
    `;
  };
      ...
      <div class="report-modal-overlay" onClick=${onCancel}>
        <div class="confirm-modal" onClick=${(e) => e.stopPropagation()}>
          ...
          <div class="confirm-modal-actions">
            ...
            <button class="modal-btn confirm-delete-btn" onClick=${onConfirm}>
              ${T.delete}
            </button>
          </div>
        </div>
  ...
  const CommentModal = ({
    ...
    const timeAgo = (date) => {
      ...
      if (interval > 1)
        return Math.floor(interval) + (lang === "ar" ? " سنة" : " years ago");
      interval = seconds / 2592000;
      if (interval > 1)
        return Math.floor(interval) + (lang === "ar" ? " شهر" : " months ago");
      interval = seconds / 86400;
      if (interval > 1)
        return Math.floor(interval) + (lang === "ar" ? " يوم" : " days ago");
      interval = seconds / 3600;
      if (interval > 1)
        return Math.floor(interval) + (lang === "ar" ? " ساعة" : " hours ago");
      interval = seconds / 60;
      if (interval > 1)
        return (
          Math.floor(interval) + (lang === "ar" ? " دقيقة" : " minutes ago")
        );
                          ...
                          (comment) => html`
                            <li class="comment-item" key=${comment.id}>
                              <div class="comment-author-avatar">
                                ${(profilesDict[comment.user_id] || comment.users?.email || "U").substring(0, 1).toUpperCase()}
                              </div>
                              <div class="comment-content-wrapper">
                                <div class="comment-meta">
                                  <span class="comment-author-name"
                                    >${profilesDict[comment.user_id] || comment.users?.email || "Anonymous"}</span
                                  >
                                  <div>
  ...
  const allVisibleSelected =
    ...
    filteredAndSortedResumes.every((r) => selectedResumeIds.has(r.id));
  const resumesToScoreCount =
    selectedResumeIds.size > 0
              ...
              resumes.length > 0 || isLoading
                ? html`
                    <div class="results-header">
                      <h2>
                        ${T.resultsHeader(filteredAndSortedResumes.length)}
                      </h2>
                    ...
                    <div class="filter-controls">
                      <div class="job-filter-group">
                        <input
                          type="text"
                          placeholder=${T.filterJobPlaceholder}
                          value=${filters.job}
                          onInput=${(e) => setFilters((f) => ({ ...f, job: e.target.value }))}
                        />
                        <label class="exact-match-label">
                          <input
                            type="checkbox"
                            checked=${jobFilterExactMatch}
                            onChange=${(e) => setJobFilterExactMatch(e.target.checked)}
                          />
                          ${T.exactMatch}
                        </label>
                      </div>
                      <input
                        class="filter-group"
                        type="text"
                        placeholder=${T.filterGovernoratePlaceholder}
                        value=${filters.governorate}
                        onInput=${(e) => setFilters((f) => ({ ...f, governorate: e.target.value }))}
                      />
                      <input
                        class="filter-group"
                        type="text"
                        ...
                        value=${filters.age}
                        onInput=${(e) => setFilters((f) => ({ ...f, age: e.target.value }))}
                      />
                      <input
                        class="filter-group"
                        type="text"
                        ...
                        onInput=${(e) => setFilters((f) => ({ ...f, skills: e.target.value }))}
                      />
                    </div>
                    ...
                    <div class="resume-list">
                      ${filteredAndSortedResumes.map((resume) => {
                        ...
                        return html`
                          <div
                            class=${`resume-card ${isSelected ? "selected" : ""}`}
                            key=${resume.id}
                          >
                          ...
                          html`
                            ...
                            >
                              <div
                                class=${`match-score-badge ${getScoreClass(scoreData.score)}`}
                              >
                                ${scoreData.score}
                              </div>
                            </div>
                          `
                              ...
                              <div class="card-actions">
                                ...
                                <button
                                  onClick=${() => {
                              setCurrentResumeForComments(resume);
                              setIsCommentModalOpen(true);
                                ...
                                <button
                                  ...
                                  class=${`icon-btn favorite-btn ${resume.is_favorited ? "favorited" : ""}`}
                                  title=${resume.is_favorited ? T.unfavorite : T.favorite}
                                  disabled=${!!cardAction}
                                >
                                ...
                                <div
                                  class="move-cv-wrapper"
                                  ref=${movingResumeId === resume.id ? moveDropdownRef : null}
                                >
                                  <button
                                    onClick=${() => setMovingResumeId(resume.id === movingResumeId ? null : resume.id)}
                                    class="icon-btn move-cv-btn"
                                    title=${T.moveCV}
                                    disabled=${!!cardAction}
                                  >
                                    ${
                                cardAction === "moving"
                                  ? html`<div class="loader card-loader"></div>`
                                  : html`<svg
                                      xmlns="http://www.w3.org/2000/svg"
                                      viewBox="0 0 24 24"
                                      fill="none"
                                  ...
                                  ${
                              movingResumeId === resume.id &&
                              html`<div class="move-dropdown">
                                <ul class="move-dropdown-list">
                                  <li
                                    class=${`move-dropdown-item ${!resume.folderId ? "disabled" : ""}`}
                                    onClick=${() => handleMoveResume(resume.id, null)}
                                  >
                                    ${T.unassigned}
                                  </li>
                                  ${folders.length > 0 ? folders.map((folder) => html`<li class=${`move-dropdown-item ${resume.folderId === folder.id ? "disabled" : ""}`} onClick=${() => handleMoveResume(resume.id, folder.id)}>${folder.name}</li>`) : html`<li class="move-dropdown-item disabled">${T.noFolders}</li>`}
                                </ul>
                              </div>`
                            }
                                </div>
                                <button
                                  ...
                                  class="icon-btn download-cv-btn"
                                  title=${T.downloadCV}
                                  disabled=${!!cardAction}
                                >
                                  ${
                              cardAction === "downloading"
                                ? html`<div class="loader card-loader"></div>`
                                : html`<svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 24 24"
                            ...
                            <div class="card-body">
                              <p>
                                <strong>${T.age}:</strong>
                                ${resume.age || T.unspecified}
                              </p>
                              <p>
                                <strong>${T.governorate}:</strong>
                                ${resume.governorate || T.unspecified}
                              </p>
                              <p>
                                <strong>${T.appliedFor}:</strong>
                                ${resume.appliedFor || T.unspecified}
                              </p>
                          ...
                          html`
                            <div class="card-expanded-details">
                              <p>
                                ...
                                ${resume.aiSummary || T.unspecified}
                              </p>
                              ${(() => {
                              const renderSkills = Array.isArray(resume.skills)
                                ? resume.skills
                                : typeof resume.skills === "string"
                                  ? resume.skills.split(",")
                                  : [];
                              return renderSkills.length > 0
                                ? html`<div
                                    class="skills-container"
                                    style=${{ marginTop: "1rem" }}
                                  >
                                    <ul class="skills-list">
                                      ${renderSkills.map((skill) => html`<li class="skill-tag">${skill}</li>`)}
                                    </ul>
                                  </div>`
                                : "";
                            })()}
                ...
                : html`<div class="placeholder">
                    ...
                    <p>
                      ${selectedFolderId === "all" ? T.placeholderText : T.placeholderFolderText}
                    </p>
                  </div>`
            }
        </div>
...
export default function CVBankPage({ session, profile }) {
  ...
  useEffect(() => {
    ...
    const checkAccess = async () => {
      try {
        ...
        console.log(
          "CVBank Access Check - Subscription Data:",
          data,
          "Error:",
          subError,
          "Role:",
        ...
        if (
          ...
          (data.plan === "pro" ||
            data.plan === "premium" ||
            data.plan === "price_pro" ||
            data.plan === "price_enterprise")
        ) {
          console.log("CVBank Access Check - Access Granted! (plan match)");
          setHasAccess(true);
        } else {
          console.log(
            "CVBank Access Check - Access Denied! Plan doesn't match Premium/Pro.",
          );
        }
      } catch (err) {
      ...
      >
        <div className="text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400">
            This feature is strictly available for HR accounts.
          </p>
        </div>
      </div>
    `;
  ...
  if (!hasAccess) {
    return html`
      ...
      >
        <div
          className="bg-white dark:bg-slate-900 shadow-xl rounded-2xl p-8 max-w-md w-full text-center border border-gray-100 dark:border-slate-800"
        >
          <div
            className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-900/40 mb-6"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-6">
            The AI-Powered CV Bank is exclusively available on
            <strong className="text-gray-900 dark:text-gray-200">Pro</strong>
            and
            <strong className="text-gray-900 dark:text-gray-200"
              >Enterprise</strong
            >
            plans. Upgrade now to unlock automated candidate analysis and match
            scoring.
          </p>
          <a
            href="/pricing"
            className="inline-flex w-full justify-center items-center rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md hover:bg-indigo-500 transition-colors"
          >
